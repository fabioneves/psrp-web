...
export function readGamepad(pad, options = {}) {
  ...
  const axis = (index, invert = false) => {
    const value = clamp(pad.axes[index], -1) * (invert ? -1 : 1);
  ...
  if (nintendo(pad)) {
    ...
    return {
      ...
      left: { x: axis(0), y: axis(1, options.invertY) }, right: { x: axis(2), y: axis(3, options.invertY) },
...
export function pollGamepads(state, enabled, settings, report, environment = globalThis) {
  let signature = '', status = '', preview = '', timer, current = null;
  const tick = () => {
    ...
    try {
      if (typeof environment.navigator.getGamepads !== 'function') next = 'Gamepad API unavailable. Try HTTPS or attach another device.';
        ...
        current = pad;
        if (pad) next = `Controller ${pad.index}: ${pad.id}${pad.mapping !== 'standard' ? ' · nonstandard mapping' : ''}${pad.vibrationActuator ? ' · rumble' : ''}${active ? '' : environment.document?.hasFocus?.() === false ? ' · click the page to send input' : ' · input starts with playback'}`;
  ...
  return {
    ...
    rumble(strong, weak) {
      ...
      if (!actuator || settings().rumble === false) return false;
      if (!strong && !weak) { actuator.reset?.(); return true; }
      actuator.playEffect?.('dual-rumble', { startDelay: 0, duration: 400, strongMagnitude: clamp(strong), weakMagnitude: clamp(weak) })?.catch?.(() => {});
      return true;
    }
  };
}
