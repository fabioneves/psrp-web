...
export function createVideoSource({ frameIntervalMs, isKey, requestKeyframe, deliver, now }) {
  const reassembler = createReassembler({ frameIntervalMs, isKey, requestKeyframe, ...(now ? { now } : {}) });
