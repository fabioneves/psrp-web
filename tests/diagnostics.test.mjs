...
test('stream log records stalls, superseded frames, underruns and console loss from metric deltas', () => {
  ...
  assert.deepEqual(log.events.map(event => event.type), ['frame-gap', 'superseded', 'decode-backlog', 'audio-underrun', 'server-loss']);
  assert.equal(log.events[0].t, 1000);
  assert.match(describeEvent(log.events[0]), /frame gap 68 ms/);
  assert.match(describeEvent(log.events[3]), /underrun ×3/);
