...
Open `/probe.html` to check whether a browser and its network could use a WebRTC video transport (see [spec/webrtc.md](spec/webrtc.md)): it reports data channel support, whether outbound UDP gets a STUN answer (from Google's and Cloudflare's public servers, or `?stun=host:port`), and how much of a 30 Mbps unreliable data channel load the browser itself can receive. **Send to server** saves the result for a signed-in browser with the diagnostics captures as `kind: "webrtc-probe"`.
...
Under Advanced, **Video output** defaults to the canvas worker path where a worker is available and to a
video element fed through a MediaStream where it is not (Safari); the element can also be chosen
explicitly. Cellular captures on a 60 Hz Mac showed the element path delivering only 43 of 60 frames to
the screen, because its per-frame write costs main-thread time, while the canvas worker path held 60. The element reports when
