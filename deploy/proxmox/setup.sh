...
say "  Update          pct exec $CTID -- /usr/local/bin/psrp update      (refuses while a stream is running)"
say "  Status / logs   pct exec $CTID -- /usr/local/bin/psrp status  ·  pct exec $CTID -- /usr/local/bin/psrp logs"
say "  Tesla           open the public address in the car and use 'Open in Tesla theater' for fullscreen"
say "Next: open the address, create the owner account (sign-ups close after it), then pair the console with Sign in to PSN."
